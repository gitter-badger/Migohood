...
@section('title')
  Location - {{ $office->type }}, {{ $office->accomodance }}
@stop
@section('menu')
  <ul>
    <a href="{{ route('office.router', ['hash' => $office->hash, 'route' => 'basics' ]) }}"><li><i class="material-icons left">library_books</i>Basics</li></a>
    <a href="{{ route('office.router', ['hash' => $office->hash, 'route' => 'location' ]) }}"><li class="active"><i class="material-icons left">location_on</i>Location</li></a>
...
@section('content')
  ...
  <!-- Uddate office Location -->
  <form class="form row" form action="{{ route('office.router.update', ['hash' => $office->hash, 'route' => 'location' ]) }}" method="POST">
    {{ csrf_field() }}
    ...
    <select name="country" required>
      <option value="" disabled selected>Choose one</option>
        @foreach($countries as $country)
          <option value="{{ $country->name }}" @if($office->country == $country->name ) selected='selected' @endif> {{ $country->name }} </option>
        @endforeach
    </select>
    <label>Country</label>
  </div>
